...
/**
 * The Burrows–Wheeler transform is the last column in the sorted suffixes array t[] of the original string,
 * preceded by the row number first (the first suffix index) in which the original string ends up.
 * 
...
public class BurrowsWheeler {
     ...
     * given a string s, generate first and t[i] (last letters of sorted suffixes)
     */ 
	public static void transform() {
    	...
    	for (int i = 0; i < sf.length(); i++) {
    		...
    		else t[i] = s.charAt(sf.index(i)-1);
    	}
     ...
     * given first and t[i] decode - recover the original string s
	...
	public static void inverseTransform() {
    	...
    	for (int i = 0; i < t.length; i++) {
    		...
    		aux[index] = ch;
    		next[index] = i; // next equals the position i of current char (ch) in unsorted t[i] array (it is the last in that suffix)
    		count[t[i]]++;
    	}
